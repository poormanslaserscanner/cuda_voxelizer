...
inline bool checkBit(unsigned int* voxel_table, size_t index){
...
void voxelize_triangle(voxinfo info, float* triangle_data, unsigned int* voxel_table, bool morton_order){
	...
	size_t gridsiz2 = info.gridsizex*info.gridsizey;
#pragma omp parallel for
	for (int triang_index = 0; triang_index < info.n_triangles; ++triang_index){ // every thread works on specific triangles in its stride
		size_t t = size_t(triang_index) * 9; // triangle contains 9 vertices
		...
		t_bbox_grid.min = glm::clamp(glm::ivec3(floor(t_bbox_world.min)),
			glm::ivec3(1, 1, 1), 
			glm::ivec3(int(info.gridsizex-1), int(info.gridsizey-1), int(info.gridsizez-1)));
		...
		for (int z = t_bbox_grid.min.z - 1; z <= t_bbox_grid.max.z; z++){
			for (int y = t_bbox_grid.min.y - 1; y <= t_bbox_grid.max.y; y++){
				for (int x = t_bbox_grid.min.x - 1; x <= t_bbox_grid.max.x; x++){
					...
					size_t location = size_t(x + y*info.gridsizex) + (z*gridsiz2);
...
void voxelize(const voxinfo& v, float* triangle_data, unsigned int* vtable, bool morton_code){
	...
	voxelize_triangle(v, triangle_data, vtable, morton_code);
